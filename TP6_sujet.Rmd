...
```{r}
marseille = st_transform(marseille, crs= 2154)
#On vérifie que c'est bien en Lambert
st_crs(marseille)
```
...
marseille %>% dplyr::group_by(depcom) %>% select(DISP_MED18)
...
```{r}
marseille_nona = marseille %>% filter(!is.na(DISP_MED18))
library(mapview)
mapview(marseille_nona, alpha.region = 0.35, label="code")
marseille_nona %>%  group_by(DISP_MED18) %>% mapview(zcol = "DISP_MED18")

plot(marseille_nona["DISP_MED18"]) #analyse continu
plot(marseille_nona["DISP_MED18"], breaks= "jenks")
plot(marseille_nona["DISP_MED18"], breaks= "pretty")
plot(marseille_nona["DISP_MED18"], breaks= "quantile")
# test de fisher
...
```{r}
set.seed(1793)
marseille_nona$DISP_MED18_ALEA = sample(marseille_nona$DISP_MED18)
...
```{r}
mapview(marseille_nona, z=c("DISP_MED18_ALEA"),alpha.region = 0.35,layer.name = "Revenu_median", label="code")

#Pour avoir deux graphiques cote à cote
plot(marseille_nona[c("DISP_MED18", "DISP_MED18_ALEA")], border= FALSE)

#La répartition n'est pas tout à fait la même.
```

```{r}
library(gridExtra)

map1 <- ggplot(marseille_nona) + 
  geom_sf(aes(fill = DISP_MED18, col = DISP_MED18)) + #col : bordure des polygone
  guides(col = "none") + #on veut pas que la bordure s'affiche
  scale_fill_viridis_c("ORIG") + 
  scale_color_viridis_c("") + 
  theme_void()
map2 <- ggplot(marseille_nona) +
...
>Un phénomène est autocorrélé spatialement quand la valeur de la variable étudiée à un endroit donné est plus liée aux valeurs de ses voisins plutôt qu'à celles des autres. 
...
Ici il s'agirait d'une autocorrélation positive.

b. Pour étudier le phénomène, il nous faut construire une matrice de voisinage. Il existe plusieurs façons de définir le voisinage de nos iris. Dans un premier temps, nous allons définir le voisinage par la contiguïté : 
deux iris sont voisins s'ils sont contigus.

Pour limiter la taille des objets créés, nous allons travailler avec des listes plutôt qu'avec des matrices carrées.

Extraire la liste des voisins de chaque Iris. Pour cela, vous utiliserez la fonction `spdep::poly2nb()`. Par défaut, il s'agit de la contiguité  dite `QUEEN` qui reprend les mouvements de la Reine aux échecs.
Prenez connaissance de l'objet créé et réalisez un résumé de l'objet en sortie avec la fonction `summary()`.

```{r}
#Extraire la liste des voisins de chaque iris
library(spdep)
liste_voisin = poly2nb(marseille_nona)
summary(liste_voisin)
...
```{r}
liste_voisin[[4]] #4 voisins
```

7. Nous allons transformer la matrice de contiguité (partage de frontière) en une matrice de pondérations. 
...
```{r}
weight_list = spdep::nb2listw(liste_voisin, zero.policy = TRUE)
```
...
somme_pon = c()
for(i in 1:336){
  somme_pon = append(somme_pon,
                     ifelse(sum(weight_list$weight[[i]]) == 1, TRUE, FALSE))
}
summary(somme_pon)

all(sapply(weight_list$weights, sum) == 1)
```
...
```{r}
marseille_nona = marseille_nona %>% mutate(DISP_MED18_STD = scale(DISP_MED18))

mean(marseille_nona$DISP_MED18_STD)
...
```{r}
moran.plot(
  as.numeric(marseille_nona$DISP_MED18_STD),
  listw = weight_list,
...
#Gauche : the poor, pour chq iris on fait la moyenne (des revenus médians) de leurs voisins.
#Cadran droit supérieur : les iris riches entourés de riches
#Cadran droit inf : riches entourés de pauvres
#Cadran gauche inf : pauvres entourés de pauvres
```
...
```{r}
moran.test(marseille_nona$DISP_MED18_STD, weight_list, randomisation=T)
#H0 Il y a de l'aléa spatial
#Hypothèse alternative: il y a de l'autocorrélation
#Si positif et significatif -> autocorrélation positive
```

b. Interpértez le résultat obtenu : confirme-t-il ou non votre hypothèse ?

```{r}
#Ici p valeur très faible, il y a une autocorrélation.
```
...
```{r}
mars_rev_lisa = spdep::localmoran(marseille_nona$DISP_MED18, weight_list, zero.policy = T)
```
...
rouge : autocorraltion positive et assez forte
bleu foncé : autocorr negative et assez forte
